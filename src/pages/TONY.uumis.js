...
import { fetchOrdersByIds, syncOrderNumbersToStatusRecords } from 'backend/orders'
// import { prepareSheetRowsByIds, buildSheetTitle, sendRowsToGoogle } from 'backend/exportOrders'
import { exportSelectedOrders, checkOrdersExportable } from 'backend/exportOrders'

const ENDPOINT_URL = 'https://script.google.com/macros/s/AKfycbySeTJtqpD-4PlLwpJ4OZztD1VCOOThT3UnBWxvkv8EAz5HxN2YgGOfkLwjneLQG8zIrA/exec'
...
$w.onReady(() => {
    ...
    dashboard.observeState(async componentParams => {
        ...
        try {
            const syncResult = await syncOrderNumbersToStatusRecords(raw)
            console.log('ordersStatus orderNumber sync', syncResult)
        } catch (error) {
            console.error('Failed to sync order numbers into ordersStatus:', error)
        }
        const ui = raw.map(o => {
    ...
    html.onMessage(async (e) => {
        ...
        if (d.type === 'sendToSheet') {
            ...
            try {
                const chk = await checkOrdersExportable(ids)
                const blocked = Array.isArray(chk?.blocked) ? chk.blocked : []
                if (blocked.length) {
                    html.postMessage({ type: 'exportBlocked', blocked })
                    return
                }
            } catch (error) {
            ...
            if (res && res.ok) {
                const message = res.softSuccess
                    ? `Sheet "${res.sheetTitle || sheetName}" created`
                    : `Sheet "${res.sheetTitle || sheetName}" created`
                html.postMessage({ type: 'notify', level: 'success', message })
                return
            }

            // Non-ok: show an actionable error (common: Drive HTML 405 = wrong/stale deployment URL)
            const status = res?.status || ''
            const body = String(res?.body || '')
            const looksDrive = status === 405 && (/<html/i.test(body) || /docs\.google\.com|drive\.google\.com/i.test(body))
            const hint = looksDrive
                ? 'Google returned a Drive HTML page (405). Verify ENDPOINT_URL is the deployed Apps Script Web App URL ending with /exec (or /dev), not a Drive file URL, and redeploy if needed.'
                : (res?.error || `Export failed (HTTP ${status || 'unknown'})`)
            html.postMessage({ type: 'notify', level: 'error', message: hint })
        ...
        if (d.type === 'sheetDecision' && lastCtx) {
            ...
            if (res && res.ok) {
                const message = res.softSuccess
                    ? `Sheet "${res.sheetTitle || lastCtx.sheetName}" created`
                    : `Sheet "${res.sheetTitle || lastCtx.sheetName}" created`
                html.postMessage({ type: 'notify', level: 'success', message })
